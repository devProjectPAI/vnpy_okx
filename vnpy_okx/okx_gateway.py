...
# OKX SWAP or FUTURES trade volume's unit is 0.01 BTC, so we need fix the volume number
SWAP_FUTURES_VOL_FIX = 100
    ...
    def on_query_contract(self, packet: dict, request: Request) -> None:
        ...
        for d in data:
            ...
            if product == Product.SPOT:
                ...
                # int some line, the data is '' null string
                if is_float(d["ctMult"]):
                    size: float = float(d["ctMult"])
                else:
                    continue

            if not is_float(d["tickSz"]):
                continue
            if not is_float(d["minSz"]):
                continue
...
class OkxWebsocketPrivateApi(WebsocketClient):
    ...
    def send_order(self, req: OrderRequest) -> str:
        ...
        if contract.product == Product.SPOT:
            ...
            args["tdMode"] = "cross"
            # need posSide
            args["posSide"] = POSSIDE_VT2OKX[req.direction]
            args["sz"] = str(req.volume * SWAP_FUTURES_VOL_FIX)  # OKX use 0.01 as the unit of volume, so we need multiply 100
...
def parse_order_data(data: dict, gateway_name: str) -> OrderData:
    ...
    fix_volume: float = 0.0
    product: Product = PRODUCT_OKX2VT[data["instType"]]
    if product == Product.SPOT:
        fix_volume = float(data["sz"])
    else:
        fix_volume = float(data["sz"])/SWAP_FUTURES_VOL_FIX
